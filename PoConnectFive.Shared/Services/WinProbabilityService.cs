...
namespace PoConnectFive.Shared.Services
{
    public class WinProbabilityService
    {
        ...
        private int QuickScoreSequence(List<int> sequence, int playerId, int opponentId)
        {
            ...
            // Quick scoring
            if (playerCount == 4 && emptyCount == 1) return 100;
            if (playerCount == 3 && emptyCount == 2) return 20;
            if (playerCount == 2 && emptyCount == 3) return 5;

            if (opponentCount == 4 && emptyCount == 1) return -100;
            if (opponentCount == 3 && emptyCount == 2) return -20;
            if (opponentCount == 2 && emptyCount == 3) return -5;
