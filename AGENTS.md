...
DO NOT create .MD files or PS1 files during our conversation. I want all action to be run one line at a time (except the azure deployment files)
...
Make sure any UI created will work well on a desktop PC as well as a mobile phone / use responsive design to ensure the layout looks good on a mobile phone in portrait mode
...
Execution and Documentation Directive: Always create a easy to test set of API methods that can be run in the swagger UI for easy testing of specific functionality
Create E2E tests with Playwright MCP as needed to cover the main application functionality using javascript
For all tests that use a database have a setup/teardown pattern so no lingering data is left after running tests
